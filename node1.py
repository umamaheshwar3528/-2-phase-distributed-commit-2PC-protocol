...
class N1:
    ...
    def setting(self, idreq, case, number, instance):
        ...
        self.file = open('node1.log', 'a+')
        self.file.write("\nSetting " + str(instance)) #self.ins(instance)
    ...
    def up_res(self, idreq, case, number, instance):
        ...
        if flag and instance not in self.abortjson:
            ...
            true_thread = Thread(target=self.trans_controller.req, args=(True, "1server" + str(instance),))
            true_thread.start()
        else:
...
def main():
    try:
        ...
        print("server listening on 5001")
        s1 = N1()
        registery = xmlrpc.client.ServerProxy("http://localhost:5003")
        s1.trans_controller = registery
        server.register_instance(s1)
